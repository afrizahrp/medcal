...
import {
  PageHeader,
  Surface,
  EquipmentUnitFilters,
  EquipmentUnitTable,
  EquipmentUnitEmptyState,
  PaginationBar,
} from "./equipment-units-ui";
import { useEquipmentUnits } from "./use-equipment-units-query";

const URL_KEYS = ["search", "sortBy", "sortDir", "page", "pageSize"] as const;

export default function EquipmentUnitsPageClient() {
  ...
  const error = query.isError && !forbidden ? "Gagal memuat daftar Equipment Unit." : null;
    ...
    <div className="w-full px-4 py-6 md:px-6 md:py-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <PageHeader
          ...
          crumbs={[{ href: "/", label: "Dashboard" }, { label: "Equipment Unit" }]}
        />

        {capabilities.equipmentCreate ? (
          <Button asChild className="shrink-0">
            <Link href="/equipment-units/new">
      ...
      <Surface className={cn("mt-6 p-4 md:p-6", fetching && "opacity-70")}>
        ...
        <EquipmentUnitFilters searchInput={searchInput} onSearchChange={setSearchInput} />
        ...
        {loading ? (
          ...
          <>
            <div className="mt-4">
              <EquipmentUnitTable units={result.data} sort={sort} />
            </div>
            <PaginationBar
              className="mt-4"
              page={page}
