"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { Plus } from "lucide-react";
import { isForbidden } from "@medcal/shared";
import { useAuthz } from "@medcal/auth/client";
import { Button } from "@/components/ui/button";
import { useDebouncedValue } from "@/hooks/use-debounced-value";
import { useUrlQueryState } from "@/hooks/use-url-query-state";
import { useTableSort } from "@/hooks/use-table-sort";
import { cn } from "@/lib/utils";
import { AccessDenied } from "../../../components/access-denied";
import {
  PageHeader,
  Surface,
  DeviceFilters,
  DeviceTable,
  DeviceEmptyState,
  PaginationBar,
  type DeviceStatus,
} from "./devices-ui";
import { useDevices } from "./use-devices-query";
import { useDeviceTypes } from "../device-types/use-device-types-query";
import { useCustomers } from "../customers/use-customers-query";

const URL_KEYS = [
  "search",
  "deviceTypeId",
  "customerId",
  "status",
  "sortBy",
  "sortDir",
  "page",
  "pageSize",
] as const;

export default function DevicesPageClient() {
  const { capabilities } = useAuthz();
  const { params, setParams } = useUrlQueryState(URL_KEYS);

  const deviceTypeId = params.deviceTypeId ?? "";
  const customerId = params.customerId ?? "";
  const status = (params.status as DeviceStatus | undefined) ?? "";
  const sort = useTableSort(params, setParams, "createdAt");
  const { sortBy, sortDir } = sort;
  const page = Number(params.page) || 1;
  const pageSize = Number(params.pageSize) || 10;
  const committedSearch = params.search ?? "";

  const [searchInput, setSearchInput] = useState(committedSearch);
  const debouncedSearch = useDebouncedValue(searchInput, 500);

  useEffect(() => {
    if (debouncedSearch !== committedSearch) {
      setParams({ search: debouncedSearch || undefined, page: undefined });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedSearch]);

  const typesQuery = useDeviceTypes({
    search: "",
    categoryId: "",
    isActive: "",
    sortBy: "name",
    sortDir: "asc",
    page: 1,
    pageSize: 100,
  });

  const customersQuery = useCustomers({
    search: "",
    status: "",
    sortBy: "name",
    sortDir: "asc",
    page: 1,
    pageSize: 100,
  });

  const query = useDevices({
    search: committedSearch,
    deviceTypeId,
    customerId,
    status,
    sortBy,
    sortDir,
    page,
    pageSize,
  });

  if (!capabilities?.deviceRead) {
    return <AccessDenied />;
  }

  const result = query.data;
  const loading = query.isLoading;
  const fetching = query.isFetching && !loading;
  const forbidden = isForbidden(query.error);
  const error = query.isError && !forbidden ? "Gagal memuat daftar Device." : null;
  const totalPages = result ? Math.max(1, result.totalPages) : 1;
  const hasFilters = Boolean(committedSearch || deviceTypeId || customerId || status);

  if (forbidden) {
    return <AccessDenied />;
  }

  return (
    <div className="w-full px-4 py-6 md:px-6 md:py-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <PageHeader
          title="Device"
          crumbs={[{ href: "/", label: "Dashboard" }, { label: "Device" }]}
        />

        {capabilities.deviceCreate ? (
          <Button asChild className="shrink-0">
            <Link href="/devices/new">
              <Plus className="h-4 w-4" /> Device
            </Link>
          </Button>
        ) : null}
      </div>

      <Surface className={cn("mt-6 p-4 md:p-6", fetching && "opacity-70")}>
        <DeviceFilters
          searchInput={searchInput}
          onSearchChange={setSearchInput}
          deviceTypeId={deviceTypeId}
          onDeviceTypeChange={(next) =>
            setParams({ deviceTypeId: next || undefined, page: undefined })
          }
          deviceTypes={typesQuery.data?.data ?? []}
          customerId={customerId}
          onCustomerChange={(next) => setParams({ customerId: next || undefined, page: undefined })}
          customers={customersQuery.data?.data ?? []}
          status={status}
          onStatusChange={(next) => setParams({ status: next || undefined, page: undefined })}
        />

        {error ? <p className="mt-4 text-sm text-red-600">{error}</p> : null}

        {loading ? (
          <p className="mt-6 text-sm text-slate-400">Memuat…</p>
        ) : result && result.data.length === 0 ? (
          <DeviceEmptyState
            onClearFilters={
              hasFilters
                ? () => {
                    setSearchInput("");
                    setParams({
                      search: undefined,
                      deviceTypeId: undefined,
                      customerId: undefined,
                      status: undefined,
                      page: undefined,
                    });
                  }
                : undefined
            }
            createHref={!hasFilters && capabilities.deviceCreate ? "/devices/new" : undefined}
          />
        ) : result ? (
          <>
            <div className="mt-4">
              <DeviceTable devices={result.data} sort={sort} />
            </div>
            <PaginationBar
              className="mt-4"
              page={page}
              totalPages={totalPages}
              total={result.total}
              pageSize={pageSize}
              itemLabel="device"
              onPageChange={(next) => setParams({ page: next <= 1 ? undefined : String(next) })}
              onPageSizeChange={(next) =>
                setParams({ pageSize: next === 10 ? undefined : String(next), page: undefined })
              }
            />
          </>
        ) : null}
      </Surface>
    </div>
  );
}
