...
### 7. MVP billing documents — Invoice + Payment + Credit Note (LOCKED)

**Keputusan:** Set dokumen keuangan MVP = **Invoice + Payment + Credit Note**.

- **Debit Note** = later.
- **Refund** sebagai entity khusus = later (boleh dicatat sementara sebagai
  catatan / payment bertanda refund bila perlu operasional).
- Interaksi Certificate ↔ tagihan setelah terbit:
  - Certificate boleh `revoked` / `superseded`
  - Jika `billingStatus` sudah `invoiced`: **tidak** mundur otomatis;
    Invoice/Payment **tidak** di-void otomatis
  - Koreksi nilai lewat **CreditNote** eksplisit oleh finance
  - Certificate pengganti (`superseded` → certificate baru) adalah record
    baru yang bisa `billable` dan masuk invoice lain bila perlu

**Alasan:** Invoice jarang berdiri sendiri di AR; Credit Note menutup edge
case sertifikat salah setelah ditagih tanpa merusak audit trail. Debit Note
& Refund ditunda demi KISS sampai volume memaksa.
...
- `web-api` (Express) menyimpan `COMPANY_ID` sebagai env var wajib per
  deployment; tidak menerima companyId dari request body.
- `LeadsModule` (Nest) memiliki dependency read-only ke CRM (D05) untuk
  proses matching email domain sebelum create, dengan blocklist domain publik.
- `BillingModule` (Nest) mendesain:
  - `Invoice` M:N ke `Certificate` (billable path)
  - `Payment`
  - `CreditNote` (koreksi; termasuk revoke/supersede after invoiced)
  - Debit Note / Refund entity = di luar MVP
- `WorkOrder` bukan billable SoR; referensi WO di invoice opsional (audit).
- `QuotationModule` mencatat `QuotationSource` dan menegakkan
  "no WO without approved Quotation".
- `CertificateModule` memegang `CertificateBillingStatus` sebagai SoR billing.
- `business-domain.md` dan `entity-catalog.md` selaras dengan keputusan #1–#7.

### 8. Inbound messaging — ContactMessage + GetMessageFrom (LOCKED)

**Keputusan:** Intake website medcal mengikuti pola existing di
`server-bi-erp` (`ContactMessage` + enum `GetMessageFrom`), bukan enum
channel baru.

**GetMessageFrom:** `CONTACTFORM` | `WHATSAPP` | `CHAT_AI` | `CHAT_PERSON` | `EMAIL`

**ContactStatus:** `PENDING` | `READ` | `REPLIED` | `CLOSED`

- Entity capture = **ContactMessage** (LeadSubmission hanya alias dokumen lama).
- `Lead` tetap ada untuk pipeline qualify/convert CBMS.
- Stack penuh `ChatSession` / mailbox `Email` di-port bertahap; MVP cukup
  ContactMessage (+ getFrom).
- Extension medcal: `matchStatus` + CRM dedup (blocklist email publik) pada
  ContactMessage.

**Refinement policy (LOCKED):** boleh ada penyesuaian tipis untuk CBMS
**tanpa mengubah core** implementasi messaging bi-erp.

| Boleh (refinement) | Jangan (breaking core) |
| -------------------- | ---------------------- |
| Tambah field medcal di ContactMessage (`matchStatus`, `matchedCustomerId`, `companyId` wajib, link ke Lead) | Ganti/rename nilai `GetMessageFrom` atau pecah jadi model channel lain |
| Filter/query & UI inbox digabung ke portal medcal | Menghapus `ContactStatus` / alur PENDING→READ→REPLIED→CLOSED |
| Port ChatSession kemudian sebagai modul tambahan | Menggabungkan ChatMessage ke ContactMessage sebagai pengganti session |
| Mapping edge Express mirip bipmed/bi-erp | Menulis business lead logic di Express |

Core yang dipertahankan: bentuk `ContactMessage`, enum `getFrom`, status inbox, dan pemisahan chat realtime (`ChatSession`…) dari inbox kontak.

---

## Status

Keputusan domain + entity defaults (termasuk MVP billing set + messaging
bi-erp) locked. Langkah berikutnya: **selesaikan review ERD** lalu Prisma.
