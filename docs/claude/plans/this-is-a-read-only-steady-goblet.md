# FINAL LOCKED — BIPMED → MedCal Architecture Adoption Matrix

**Status:** Finalized architecture decision document. Research/analysis only — no code written, no MedCal or BIPMED source modified. Produced after the two remaining open points below were resolved; no further exploration was performed per instruction.
...
**If this ever needs to change** (e.g., the two surfaces need genuinely divergent tech stacks, wildly different scaling profiles, or separate team ownership), that is itself a new ADR at that time — the same pattern MedCal already uses for reversing other "not now" decisions (see Branch) — not something to pre-build for today.
...
- **Not mergeable into a real account automatically.** If the visitor becomes a lead/customer, that happens through the already-locked, explicit `ContactMessage`/email-domain-match-plus-admin-confirmation flow — never an automatic "promote this anonymous session to a user" step. This is a deliberate difference from the anonymous-session-merge pattern observed earlier in `server-ngebengkel` (flagged there only as a thematically similar shape, never adopted) — worth restating explicitly so the two are not conflated later.

**How this connects to the already-decided Chat WebSocket architecture:** the WS gateway's `handleConnection` now has two validation branches, not one — if the connecting client presents a Better Auth session cookie, validate via the in-process `auth.api.getSession()` (staff/authenticated path, unchanged from the prior review); if it presents (or needs to be issued) a `ChatSessionToken` instead, validate/issue via the narrow chat-continuity check above (anonymous visitor path). Both branches terminate in the same `ChatSession`/`ChatMessage` domain model — the two conversation participants (visitor and staff) are simply authenticated by different, appropriately-scoped mechanisms.

**Why this satisfies "deliberate narrow exception, not a reversal":** it creates no IAM identity, no cross-conversation visitor profile, no persistent tracking beyond one conversation's lifetime, and cannot be used for anything Better Auth is responsible for. "No durable anonymous user" remains true for the actual identity/authentication system; this mechanism never touches that system at all.

**`ChatSession`/`ChatMessage` schema implication:** no new fields beyond what a `ChatSessionToken`-to-`ChatSession` mapping requires (e.g., a hashed token column or a join, at implementation time — not specified further here per "do not implement"). This does not reopen or require the previously-rejected `mode` field.
...
# BIPMED → MedCal Architecture Adoption Matrix (FINAL)
...
| Capability | BIPMED Pattern | What Works | Problem/Risk | MedCal Decision | Target Approach | Phase | Dependencies |
|---|---|---|---|---|---|---|---|
| WhatsApp flow | Identity capture → persist as `ContactMessage` (CAPTCHA skipped) → `wa.me` handoff | Simple, no infra dependency | CAPTCHA bypass is a proven spam vector | **ADOPT** the handoff shape; **ADAPT** the spam gap | Same capture-then-handoff flow; do not skip CAPTCHA/rate-limit for WhatsApp-sourced submissions the way BIPMED does | MVP | Lead/Contact intake |
...
| Staff WS auth | N/A (BIPMED's WS server has its own security layer, separate from dashboard auth) | — | — | **ADOPT the principle, redesign the mechanism** | In-process `auth.api.getSession()` at `handleConnection`, same as REST | MVP | — |
...
| Capability | BIPMED Pattern | What Works | Problem/Risk | MedCal Decision | Target Approach | Phase | Dependencies |
|---|---|---|---|---|---|---|---|
| Outbound reply persistence | SMTP fire-and-forget, no `Email` row created for replies | Simple | Can't analyze past conversations later | **DEFER, flagged** | MVP follows the same fire-and-forget pattern (MedCal's own `business-domain.md` D18 already treats this as Later: "log optional") — accepted as a known limitation of the current MVP scope, not silently inherited without notice | Later | — |
...
| Capability | BIPMED Pattern | What Works | Problem/Risk | MedCal Decision | Target Approach | Phase | Dependencies |
|---|---|---|---|---|---|---|---|
| UTM/referrer/source capture | None at all in BIPMED | — | Genuinely absent, not just under-documented | **DEFER capture, REDESIGN schema-readiness** | No UTM capture logic in MVP; `ContactMessage`/`Lead` schema keeps room for it (flexible field) so it isn't foreclosed later | Later (schema Foundation-ready) | — |
...
| Mobile-first | Not evaluated in BIPMED (desktop-oriented dashboard) | — | — | **REDESIGN** | Mobile → Tablet → Desktop enhancement for every surface; desktop-first-class treatment reserved for Dashboard/Reports/Admin/Finance specifically, per MedCal's own already-authored guidelines | Foundation | — |
