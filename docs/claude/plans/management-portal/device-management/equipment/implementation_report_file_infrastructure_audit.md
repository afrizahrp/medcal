# File Infrastructure Architecture Audit — Existing `FileObject` → Minimal Evidence File Capability

**Date:** 2026-08-29
**Author:** afriza.hrp@gmail.com (via Claude Code)
**Source brief:** "MEDCAL — Focused File Infrastructure Architecture Audit" (pasted task)
**Related:** `implementation_report_equipment_phase2a.md` (Equipment master — done),
`implementation_report_equipment_phase2b_audit.md` (EquipmentCalibrationRecord + evidence — approved conceptually)
**Mode:** AUDIT / DISCOVERY ONLY.

> ### Explicit confirmation — nothing was modified
...
> are untouched. This report is the only artifact.

Findings basis: 3 parallel read-only codebase explorations + direct reads of
...
Two implementation decisions were confirmed with the product owner during this audit:
**(a)** the recommended Phase-1 store is **local disk on a bind-mounted host directory**
(behind a `StorageDriver` interface so MinIO/S3 stays a later drop-in); **(b)** the
`FileObject` schema changes are the **minimal safe set** — `checksum`, `uploadedByUserId`,
`updatedAt`, plus the `EQUIPMENT_CALIBRATION` enum value.
...
needs **4 small additions**: `checksum` (sha256), `uploadedByUserId`, `updatedAt`, and one
`FileOwnerType` value (`EQUIPMENT_CALIBRATION`) — plus `@@unique([storageKey])`.

**Recommended storage: local disk on a bind-mounted host directory**, added to
...
| **`checksum` (sha256 hex)** | **REQUIRED (add)** | integrity — prove the stored bytes are the bytes that were uploaded; detect silent corruption / restore mismatch; de-dupe check. Central to "evidence." |
| **`uploadedByUserId` (FK → User, nullable)** | **REQUIRED (add)** | attribution — audit Q ("who provided this evidence"). Nullable for system-generated files. |
| **`updatedAt`** | **REQUIRED (add)** | `FileObject` has none; needed the moment any field (e.g. a soft-delete marker) can change. |
...
| | **Option A — Local disk + `FileObject` metadata** *(RECOMMENDED, confirmed)* | **Option B — Self-hosted MinIO + `FileObject` metadata** | **Option C — Reuse existing infrastructure** |
|---|---|---|---|
...
**Confirmed with the product owner: Option A — local disk on a bind-mounted host directory,
behind a `StorageDriver` interface so MinIO/S3 is a documented drop-in growth path.**
...
## 27. Explicit confirmation that NOTHING was modified

See the boxed statement at the top. **No code, schema, migration, database row, API,
service, controller, DTO/Zod schema, RBAC catalog, seed script, storage configuration,
Docker/compose file, Nginx config, or UI file was created or modified.** No file upload or
download was built. This audit report is the only artifact.
