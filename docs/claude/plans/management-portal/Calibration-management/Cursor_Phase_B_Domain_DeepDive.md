...
## IMPORTANT CONTEXT UPDATE (read before starting)
Only the Customer stage is actually implemented end-to-end today. Calibration Request,
Quotation, Purchase Order, Work Order, Invoice, and Payment are EXPECTED to be mostly
"Missing entirely" or "Planned-Only" at the code level right now — this is known and is NOT
itself a finding worth flagging repeatedly. What actually matters for this audit is the
FOUNDATION that already exists (Prisma schema, DocumentNumberService, enums, planning docs)
and whether it is sound enough to safely build the missing pieces on top of.

For every finding, in addition to the normal fields, classify it as ONE of:
- `Not Yet Built (expected)` — a module/route/service simply doesn't exist yet, consistent
  with the known roadmap. Log it once per stage for completeness, do not elaborate further,
  and do NOT assign it Blocker-for-MVP (use Nice-to-have or omit MVP Relevance for these).
- `Foundation Issue` — something in the parts that DO already exist (schema, enums,
  DocumentNumberService, planning docs) has a defect, gap, or inconsistency that will cause a
  real problem once the missing module is built on top of it (e.g. a document type missing
  from an enum, a required relation absent from the schema, a planning doc that contradicts
  the schema). These DO get a real MVP Relevance tag and deserve full evidence + explanation.

Spend most of your analysis effort on `Foundation Issue` findings — that is the actual value
of this phase given current implementation status.
