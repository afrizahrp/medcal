...
- IMPORTANT — this changes what the verdict means: only Customer is fully implemented today.
  All other stages are expected/known to be largely unbuilt. This audit is therefore a
  READINESS-TO-BUILD audit, not a production-readiness audit. The verdict must answer: "is the
  existing foundation (schema, DocumentNumberService, enums, planning docs) sound enough to
  safely start building the missing pieces on top of it?" — NOT "is everything built and
  working?" (the honest answer to that would trivially be no, and that's already known).
- When compiling Sections 14-17, ONLY findings classified as `Foundation Issue` in Phase B/C
  count toward the verdict and risk buckets. Findings classified as `Not Yet Built (expected)`
  should be summarized in ONE short list (e.g. in Section 2 or an appendix note) — do not
  scatter them across Blockers/High/Medium/Low, and do not let their sheer volume drag the
  verdict down. A missing module is not a defect; an unsound foundation underneath a module
  that's about to be built IS a defect.
...
6. Write ONE overall verdict — GREEN, YELLOW, or RED — based on `Foundation Issue` findings
   ONLY (ignore `Not Yet Built (expected)` volume entirely for this decision):
   - RED = 1 or more Foundation Issue findings tagged Blocker-for-MVP that would corrupt
     correctness once built on (e.g. a numbering enum gap that blocks Invoice/Payment
     numbering from working at all, a schema relation that can't represent required
     cardinality, a documented plan that contradicts the actual schema).
   - YELLOW = no Blocker-for-MVP Foundation Issues, but several unresolved High-Risk
     Foundation Issues that should be decided/fixed before the next module is built, to avoid
     rework.
   - GREEN = no Foundation Issue blockers; existing foundation (schema, numbering, docs) is
     internally consistent enough to safely start building the next stage(s).
7. Add one short subsection near Section 2 (or as an appendix) titled "Not Yet Built (Expected
   — Informational Only)" listing, per stage, what's confirmed absent — one line each, sourced
   from Phase B/C. This is for completeness/reference, explicitly excluded from the verdict
   and from Sections 14-17.
8. Executive summary is OPTIONAL — write it LAST, keep it to 3-5 sentences max, do not spend
